...
export class DialogAddEditComponent implements OnInit {
  ...
  mostratAlerta(message: string, action: string) {
  ...
  addEmpleado(){
    ...
    if(this.dataEmpleado == null){
      this._empleadoService.add(modelo).subscribe({
        next:(res)=>{
          this.mostratAlerta("El Empleado fue creado","Listo");
          this.dialogoReferencia.close("creado")
        },error:(err)=>{
          this.mostratAlerta("No se pudo crear","Error");
        }
      })
    }else{
      this._empleadoService.update(this.dataEmpleado.idEmpleado,modelo).subscribe({
        next:(res)=>{
          this.mostratAlerta("El Empleado fue editado","Listo");
          this.dialogoReferencia.close("editado")
        },error:(err)=>{
          this.mostratAlerta("No se pudo editar","Error");
        }
      })
    }
  }
